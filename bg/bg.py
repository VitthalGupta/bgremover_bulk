...
class Net(torch.nn.Module):
    ...
    def forward(self, block_input: torch.Tensor):
        ...
        image_data = torch.nn.functional.interpolate(
            image_data, (320, 320), mode='bilinear')
        ...
        out = torch.nn.functional.interpolate(
            out, original_shape, mode='bilinear')
        out = out[:, 0]
        out = out.to(dtype=torch.uint8, device=torch.device(
            'cpu'), non_blocking=True).detach()
        return out
...
def alpha_matting_cutout(
    ...
    if erode_structure_size > 0:
        structure = np.ones(
            (erode_structure_size, erode_structure_size), dtype=np.int)
...
def remove_many(image_data: typing.List[np.array], net: Net):
    ...
    image_data = torch.as_tensor(
        image_data, dtype=torch.float32, device=DEVICE)
